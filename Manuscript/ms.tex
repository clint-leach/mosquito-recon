...
Moreover, mechanistic models can provide scenario-based tools that can be used to predict the effect of management actions \cite{Buckland2007}.
...
In addition, posterior predictive checks show that the model reproduces the total number of cases reported and mosquitoes captured as well as the autocorrelation structure of both time series (with the exception of slightly underestimating the autocorrelation for short lags, Supplemental Figures 5 - 7).
...
Modifying these estimated demographic rates to simulate targeted control efforts, we found that control applied in the third week of the year is the most effective at preventing cases of dengue fever (though weeks 2 and 4 are also very close, Figure \ref{control}A).
...
Our results suggest that applying control early in the year is critical to disrupting city-wide disease spread, but targeting the city's entire mosquito population is likely infeasible and spatial prioritization of control is also necessary.
