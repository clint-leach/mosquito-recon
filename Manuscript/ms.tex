...
We model the weekly mosquito mortality rate, $d_{vt}$, relative to $\rho_{vt}$:
\begin{equation}
d_{vt} = \rho_{vt} \exp(-\mu - \nu_t).
\end{equation}
Assuming mosquito lifespan and latent periods are exponentially distributed (as the differential equation framework implicitly does), the probability that a newly exposed mosquito will survive until it becomes infectious is then
\begin{equation}
\psi_t = \frac{\rho_{vt}}{\rho_{vt} + d_{vt}} = \text{logit}^{-1}(\mu + \nu_t).
\end{equation}
We then model $\nu_t$ and the mosquito population growth rate, $r_t$, using a (Box-Jenkins?) seasonal auto-regressive model (CITATION):
\begin{align}
\nu_{t} & = \alpha_{\nu} \nu_{t-1} + \theta_{\nu} (\nu_{t-52} - \alpha_{\nu} \nu_{t-53}) +\epsilon_{\nu t}\\
...
Several of the parameters in this model are assumed to be fixed and known. 
...
The remaining parameters include the epidemiological parameters ($\rho$, $\gamma$, $\delta$), the initial conditions of the model ($S_0$, $E_0$, $I_0$, $R_0$, $V_{N0}$), the latent mosquito process autoregressive ($\alpha_r$, $\alpha_{\nu}$, $\theta_r$, $\theta_{\nu}$) and variance ($\sigma_r$, $\sigma_{\nu}$) parameters, and the remaining measurement parameters ($\phi_q$, $\eta_y$, $\eta_q$).  
...
$\exp(\mu)$ & Mean ratio of mortality to incubation rates & 3.6 & \cite{Chan2012,Brady2013} \\
