...
Similarly, susceptible mosquitoes ($V_S$) become exposed ($V_S$) by biting infectious humans and pass through a temperature-dependent incubation period ($\frac{1}{\rho_{vt}}$) before becoming infectious ($V_I$).
Because the assumption of an exponentially distributed incubation period (implicit in the specification of a differential equation model) is a poor fit to laboratory observations \cite{Chan2012}, we instead implement a gamma-distributed incubation period by chaining multiple exposed classes together (taking advantage of the fact that the sum of a collection of exponential random variables with the same rate parameter is gamma-distributed) \cite{Lloyd2001}.
...
We found that modeling temporary human immunity and seasonal variation in the mosquito mortality rate was able to effectively fit case reports and mosquito surveillance data from Vitoria, Brazil.
Control simulations using this model indicated that mosquito control was most effective at reducing the disease burden in a given year when implemented approximately 30 weeks prior to the start of that year, when the susceptible pool tends to be at a minimum.
Moreover, the effectiveness of control was strongly correlated with the period of cross-immunity, and the rate of turnover of the susceptible population.
Thus, human immunity appears to be a critical driver of the effectiveness of mosquito control.
