...
Given its role in mediating transmission, however, mosquito mortality can be informed by a combination of mosquito and clinical surveillance, borrowing from the field of integrated population modeling \cite{Shaub2010}.
...
In our model, the relationship between mosquito abundance and human disease risk is controlled by mosquito mortality, which, together with the extrinsic incubation period ($\rho_v$), controls the probability of an exposed mosquito surviving long enough to be infectious $\left(\frac{\rho_v}{\rho_v + d}\right)$.
Since the extrinsic incubation period is forced by temperature, the time series of transmission risk is noisy, but is highest during what is generally the dengue off-season.
That these periods of high transmission risk do not correspond to periods of high disease is likely due to the feedbacks between mosquito population dynamics and human epidemiology.
Even though transmission risk is high and mosquito abundance is climbing late in the year, it takes a relatively long time for enough infection to build in the human population for an outbreak to occur.
Despite these seasonal fluctuations, transmission risk is always fairly low (less than 0.4), suggesting that throughout the year, relatively few mosquitoes may be responsible for transmission.
