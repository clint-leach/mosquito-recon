...
Modifying these estimated demographic rates to simulate targeted control efforts, we find that control (targeting either adults via increasing the death rate or larvae via decreasing the birth rate) applied in the third week of the year is the most effective at preventing cases of dengue fever (though weeks 2 and 4 are also very close, Figure \ref{control}).
In fact, increasing the mosquito death rate by just 5\% in the third week of 2009, 2010, and 2011 prevents 13,988 (reported) cases of dengue fever (with 80\% credible interval of 11,130 to 15,811).
...
It is also generally before the estimated peak in mosquito abundance, which occurs between weeks 6 and 9 (Figure \ref{timing}).
Annual peaks in estimated mosquito death rate, and infection risk (i.e. the probability that a mosquito survives long enough to become infectious, $\frac{\rho_v}{(\rho_v + d)}$) are more variable, but also do not generally line up with the timing of optimal control.

\begin{figure}[!h]
\includegraphics[angle = 270, scale = 1]{figures/fig1.eps}
\caption{{\bf Vitoria data and model fits.}
Weekly observations from 2008 to week 34 of 2012 (points), with corresponding median posterior prediction (line) and 80\% credible interval (gray band). A: reported dengue cases. B: total number of trapped mosquiotes.
}
\label{timeseries}
\end{figure}
...
Median posterior estimate (line) and 80\% credible interval.  A: weekly mosquito population growth rate ($r_t$). B: weekly mosquito death rate ($d_t$).
}
\label{latent}
\end{figure}
...
Number of reported cases of dengue fever prevented by control deployed during a given week each year in 2009, 2010, and 2011. Median posterior prediction (line) and 80\% credible interval. A: adult control, simulated by increasing death rate in the given week by 5\%. B: larval control, simulated by decreasing birth rate ($r_t + d_t$) in a given week by 5\%.
}
\label{control}
\end{figure}
...
The vertical black line indicates week 3, during which mosquito control produces the largest decline in predicted dengue case reports. Each colored line represents a different year: salmon? = 2008, green-yellow = 2009, green = 2010, blue = 2011, purple = 2012. A: posterior median estimate of dengue case reports per week. B: posterior median estimate of trapped mosquitoes per week. C: posterior median estimate of weekly mosquito death rate. D: posterior median estimate of weekly infection risk ($\frac{\rho_v}{\rho_v + d}$).
}
\label{timing}
\end{figure}
